...
async def generate_outfit(request: GenerateOutfitRequest, user: CurrentUser):
    ...
    if request.prompt and request.image_url:
        user_input.append(f"Create an outfit based on this prompt: '{request.prompt}'. Use the provided image for visual inspiration and style reference.")
    elif request.prompt:
        user_input.append(request.prompt)
    elif request.image_url:
        user_input.append("Create an outfit that perfectly matches the vibe, style, and color palette of this reference image.")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="The stylist encountered an error generating your outfit.")
